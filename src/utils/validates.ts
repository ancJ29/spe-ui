...
export const passwordSchemaValidate = () => {
  return z.string()
    .min(8, { message: _t(dictionary, "Password must be at least 8 characters") })
    .max(20, { message: _t(dictionary, "Password must be at most 20 characters") })
    .regex(/[A-Z]/, { message: _t(dictionary, "Password must contain at least one uppercase letter") })
    .regex(/[a-z]/, { message: _t(dictionary, "Password must contain at least one lowercase letter") })
    .regex(/[0-9]/, { message: _t(dictionary, "Password must contain at least one number") });
};
...
export const requiredFieldedValidate = () => {
  return z.string()
