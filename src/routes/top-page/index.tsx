...
export default function TopPage() {
  ...
  return (
    <>
      ...
      <Banner />
      <SliderNotice />
      <Box className={classes.bgtopage} style={{ "overflow": "hidden" }}>
        <Space h={50} />
        <Container size="xl">
          <Box>
            <CarouselPage />
          ...
          <Box>
            <Group justify="space-between">
              ...
              <AppButton instancetype="WithRightIcon" size="lg" color="primary" variant="light">See more</AppButton>
            </Group>
            <Space h={"md"} />
            <Grid gutter={"lg"}>
              <Grid.Col span={8}>
            ...
            <Group h="100%" gap={0} visibleFrom="sm">
              {
                menus.map((item, idx) => {
                  return (
                    <Fragment key={idx}>
                      {item.type === "group" && (<Menu variant='transparent' position="bottom-start" shadow="md" width={300} trigger='hover' offset={0}>
                        <Menu.Target>
                        ...
                        <Menu.Dropdown bg={"black"} variant="transparent" style={{ "border": "none", "borderRadius": 0 }}>
                          {item.children.map((_item: any, i: number) => (
                            <UnstyledButton className={classes.subLink} key={i} variant="transparent">
                      ...
                      {
                        item.type === "link" && (<a href="#" className={classes.link}>
                          {item.label}
                        </a>)
                      }
                      {
                              ...
                              <SimpleGrid cols={2} spacing={0}>
                                {
                                  item.children.map((_item, i) => (
                                    <UnstyledButton key={i} className={classes.subLink} variant="transparent">
                                      <Group wrap="nowrap" align="flex-start">
            ...
            menus.map((item, idx) => {
              return (
                <Fragment key={idx}>
                  {item.type === "group" && (
                    <>
                      <UnstyledButton className={cx(classes.link, classes.colorDefault)} onClick={() => toggle(`${item.label}${item.type}`, !opened[`${item.label}${item.type}`])}>
                        <Group justify="space-between" w={"100%"}>
                          <Box component="span" mr={5}>
                            {item.label}
                          </Box>
                          <IconCaretDownFilled style={{ width: rem(16), height: rem(16) }} className={cx(classes.icon, classes.translate)} />
                        </Group>
                      </UnstyledButton>
                      <Collapse in={Boolean(opened[`${item.label}${item.type}`]) === true}>
                        {
                          item.children.map((_item, i) => (
                            <UnstyledButton key={i} className={classes.subLink} variant="transparent">
                              <Group wrap="nowrap" align="flex-start">
                                ...
                                <div>
                                  <Text size="sm" fw={500} className={cx(classes.subLinkTitle, classes.defaultColor)}>
                                    {_item?.label}
                                  </Text>
                                  <Text size="xs" c="dimmed">
                                                                        Lorem ipsum dolor sit, amet consectetur adipisicing elit.
                  ...
                  {
                    item.type === "link" && (<a href="#" className={cx(classes.link, classes.colorDefault)}>
                      {item.label}
                    </a>)
                  }
                  {
          ...
          <Grid.Col span={9}>
            <Card>
              <MarqueeList speed={100}>
                {
                  items.map((_, _k) => (
                    <Card key={_k} padding="lg" radius="md" mx={5} bg={alpha("gray", 0.03)} withBorder>
              ...
              <Flex gap={10} mt={30}>
                {
                  footer.socials.map((s, i) => (
                    <a href={s.url ?? "#"} key={i} target="_blank" rel="noreferrer">
                      <Avatar color="primary" radius="sm" src={s.icon} />
                    </a>
                  ))
                }
              </Flex>
              ...
              <Grid justify="flex-end">
                {footer.groups.map((group, i) => (
                  <Grid.Col span={3} key={i}>
                    <Title order={4} mb={14} c="primary">{group.name}</Title>
                    <Group>
                      {group.links.map((link, i) => (
                        <a className={classes.foolink} key={i} href={link.url}>
      ...
      <Box>
        <Container size="xl">
          <Group justify='center' py={10}>
            ...
            <Text className={classes.hoverlink} fz={"12px"} component="a" href={footer.termOfService.url}>
              {footer.termOfService.name}
            </Text>
            <Text className={classes.hoverlink} fz={"12px"} component="a" href={footer.privacyTerms.url} target="_blank">
