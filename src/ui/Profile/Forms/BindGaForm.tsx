...
export function BindGaForm() {
    ...
    let otpauth = generateUri2FA("totp", label, secret, "spe", "100")
    return (
        <>
            <Box py={40}>
                <Timeline bulletSize={24} lineWidth={1} styles={{
                    itemBullet: {
                    ...
                    >
                        <Flex gap={50}>
                            ...
                            <Box maw={"485px"} w={"100%"}>
                                <Text fz={14}>
                                    {t("Launch Google Authenticator app and scan the QR code or enter the key.")}
                                </Text>
                                <Text fz={14}>
                                    {t("Setup key")}: <strong style={{
                                        borderBottom: "solid 1px"
                                    }}>{secret}</strong>
                                </Text>
                                <Space my={"sm"} />
                                <Text c={"primary"} fz={14}>
                                    Write down your setup key in somewhere safe so that you can regain access to your authenticator app if you lose or switch your phone.
                                </Text>
                            </Box>
                        </Flex>
                    </Timeline.Item>
                    <Timeline.Item title={
                        <Box mb={20}><Title order={3}>{t("Security Verification")}</Title></Box>
                    } bullet={
                        <Text styles={{
                            root: {
                                color: "light-dark(white, black)"
                        ...
                        <Flex gap={50}>
                            <Box>
                                ...
                                } placeholder={t("Enter the verification code")} rightSectionWidth={60} rightSection={
                                    <Flex px={10} w={"100%"}>
                                        <Button p={0} variant="transparent">{t("Send")}</Button>
                                    </Flex>
                                } />
                                <Space my={"md"}/>
                                <TextInput label={t("Google Authenticator Code")} placeholder={t("Enter 6-digit code")} />
                                <Space my={"md"}/>
                                <Button fullWidth variant="gradient" gradient={{ from: "primary", to: "yellow", deg: 90 }}>{t("Confirm")}</Button>
                            </Box>
                        </Flex>
