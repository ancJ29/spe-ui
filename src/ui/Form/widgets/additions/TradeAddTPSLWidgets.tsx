...
import { ActionIcon, Alert, Box, Button, Checkbox, Flex, Grid, Group, InputLabel, Modal, NumberInput, Radio, SegmentedControl, Select, SimpleGrid, Slider, Space, Stack, Text, TextInput } from "@mantine/core";
import { useDisclosure, useToggle } from "@mantine/hooks";
import { WidgetProps } from "@rjsf/utils";
import { IconCaretDownFilled, IconEraser, IconHelp, IconMinus, IconPlus, IconSwitch3 } from "@tabler/icons-react";
...
const calcROI = (value: number, orderPrice: number, isLong: boolean = false) => {
    if(isLong) {
...
const calcTP = (roi: number, orderPrice: number, isLong: boolean = false) => {
    if(isLong) {
...
const calcSL = (roi: number, orderPrice: number, isLong: boolean = false) => {
    if(isLong) {
        return orderPrice * (1 - roi / 100)
    }else {
        return orderPrice * (1 + roi / 100)
...
export function ProfitInputWidget(props: WidgetProps) {
    ...
    const [type] = useState<'Long' | 'Short'>(props.formContext['formData']?.type);
    ...
    const onChangePrice = useCallback((v: number) => {
        props.onChange(parseFloat(v.toFixed(5)))
        let roi = calcROI(v, orderPrice, isLong)
        setROI(roi)
    }, [props.value, ROI, isLong])

    const onChangeROI = useCallback((v: number) => {
        setROI(v)
        let tp = calcTP(v, orderPrice, isLong)
        props.onChange(parseFloat(tp.toFixed(5)))
    }, [props.value, ROI, isLong])

    const onTickPrice = useCallback((isDecrement: boolean = false) => {
        if (Boolean(props.value) && props.value > 0) {
            let v = parseFloat(props.value)
            let tp = (isDecrement ?  v - T : v + T)
            let roi = calcROI(tp, orderPrice, isLong)
    ...
    return (
        <>
            <Box>
                                    ...
                                    <NumberInput size="lg"
                                        ...
                                        rightSection={
                                            <Flex align={"center"} gap={0} justify={"end"} w={"100%"}>
                                                ...
                                                <Flex align={"center"}>
                                                    <ActionIcon variant="transparent" onClick={() => onTickPrice()}>
                                                        <IconPlus size={16} />
                                                    </ActionIcon>
                                                </Flex>
                                                <Flex flex={"0 0 60px"}>
                ...
                {isShowAlerTP && <Alert variant="light" color="gray" fz={12} p={10} styles={{
                    message: {
...
export function StopLossInputWidget(props: WidgetProps) {
    ...
    const onChangeROI = useCallback((v: number) => {
        setROI(v)
        let sl = calcSL(v, orderPrice, isLong)
    ...
    const onTickPrice = useCallback((isDecrement: boolean = false) => {
        if (Boolean(props.value) && props.value > 0) {
            let v = parseFloat(props.value)
            let sl = (isDecrement ?  v - T : v + T)
            let roi = calcROI(sl, orderPrice, isLong)
            setROI(parseFloat(roi.toFixed(2)))
            props.onChange(parseFloat(sl.toFixed(5)))
        }
    }, [props.value, isLong])
    ...
    return (
        <>
            <Box className="space-y-6">
                    ...
                    <Grid columns={12} gutter={10}>
                        <Grid.Col span={8}>
                            <NumberInput size="lg"
                                ...
                                placeholder="Trigger Price"
                                rightSection={
                                    <Flex align={"center"} gap={2} justify={"end"} w={"100%"}>
                                        <Flex align={"center"}>
                                            {isHasValue && <ActionIcon variant="transparent" onClick={() => {
                                                setValue("")
                                                props.onChange("")
                                                setROI(0)
                                            }}>
                ...
                {isShowAlerSL && <Alert variant="light" color="gray" fz={12} p={10} styles={{
                    message: {
                        color: "rgba(255,255,255,0.6)",
                        fontSize: "10px",
                        padding: "0px"
                    ...
                    <div className="text-label-alert">
                        Last Traded Price to 0.00100 will trigger market Stop Loss order; your expected profit will be 250,179.7676 USDT (ROI: 1,000.00%)
                    </div>
                </Alert>}
            </Box>
        </>
            ...
            <Alert variant="light" color="gray" fz={12} p={10} styles={{
                ...
                <div className="space-y-10">
                    <Flex gap={10} align={"flex-start"}>
                        ...
                        <span className="text-label-alert">Last Traded Price to 0.00057 USDT will trigger a Limit Take Profit order at -- USDT.</span>
                    </Flex>
                    <Flex gap={10} align={"flex-start"}>
                        <div>
                            <span className="badge-bg">2</span>
                        </div>
                        <span className="text-label-alert">Your expected profit will be -- USDT. (ROI: --%)</span>
                    </Flex>
                </div>
            </Alert>
            <Space h={10} />
            <AppText className="text-hint">
                * Limit TP/SL orders will only be filled at the specified price, but there is a risk that the order may not be filled.
            </AppText>
        </>
    )
}
            ...
            <Alert variant="light" color="gray" fz={12} p={10} styles={{
                ...
                <div className="space-y-10">
                    <Flex gap={10} align={"flex-start"}>
                        ...
                        <span className="text-label-alert">Last Traded Price to 0.00100 USDT will trigger a Limit Stop Loss order at -- USDT.</span>
                    </Flex>
                    <Flex gap={10} align={"flex-start"}>
                        <div>
                            <span className="badge-bg">2</span>
...
export function TpAndSlSettingsWidget(props: WidgetProps) {
    ...
    const optionAsLabel = useMemo(() => {
        return options.find(i => i.value.toString() === value)?.name ?? "--"
    }, [value])

    return (
        <>
            <Flex align={"center"} gap={5}>
                <AppText className="cursor-pointer" onClick={openTPSLSettings} fz={14}>
    ...
    return (
        <>
            <Flex justify={"space-between"} h={"100%"}>
                <Box></Box>
                <Flex align={"center"}>
                    <Checkbox m={0} p={0} size="xs" label="Limit" checked={checked} onChange={() => {
                        setChecked(!checked)
