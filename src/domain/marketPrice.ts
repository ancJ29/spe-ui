...
export function convertCoinToCoinUsingRate(_baseCoin: string, _pairCoin: string, marketPrices: MarketPrice) {
    let coinBase1 = `${_baseCoin}${_pairCoin}`
    let coinBase2 = `${_pairCoin}${_baseCoin}`
    
    let baseValue1 = marketPrices[coinBase1 as SWAP_SYMBOL]
    let baseValue2 = marketPrices[coinBase2 as SWAP_SYMBOL]

    let baseCoin = baseValue1 ? _baseCoin : _pairCoin
    let pairCoin = baseValue1 ? _pairCoin : _baseCoin

    let baseValue = baseValue1 ?? baseValue2
    let pairValue = bigNumber.div(1, baseValue)
    return {
      baseCoin,
      pairCoin,
      baseValue,
